...
def training_loop(
    ...
    prev_lod = -1.0
    with tqdm(total=total_kimg * 1000,initial=cur_nimg,dynamic_ncols=True) as pbar:
        while cur_nimg < total_kimg * 1000:
            if ctx.should_stop(): break
            pbar.update(cur_nimg - pbar.n)
            ...
            for _mb_repeat in range(minibatch_repeats):
                with tqdm(total=D_repeats * minibatch_repeats, position=1, desc="Discriminator Training") as pbar_d:
                ...
                pbar.set_postfix({'nimg': cur_nimg, 'lod': sched.lod})
